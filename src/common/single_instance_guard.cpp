...
std::string get_temp_dir_path() {
    ...
    if (temp_dir == nullptr) { temp_dir = "/tmp"; }
...
SingleInstanceGuard::SingleInstanceGuard(const std::string& lock_file_name) {
    ...
    for (int i = 0; i < 2; ++i) { // 最多重试一次
        ...
        if (file_handle_ != INVALID_HANDLE_VALUE) {
            ...
            DWORD bytes_written;
            WriteFile(file_handle_, pid_str.c_str(), pid_str.length(), &bytes_written, NULL);
            return; // 成功，退出构造函数
        }

        // 如果获取锁失败
        if (GetLastError() != ERROR_SHARING_VIOLATION) {
            throw std::runtime_error("Failed to create lock file for unknown reason.");
        }
...
SingleInstanceGuard::SingleInstanceGuard(const std::string& lock_file_name) {
    ...
    for (int i = 0; i < 2; ++i) { // 最多重试一次
        ...
        if (flock(file_descriptor_, LOCK_EX | LOCK_NB) == 0) {
            ...
            if (write(file_descriptor_, pid_str.c_str(), pid_str.length()) == -1) { /* ignore */ }
            return; // 成功，退出构造函数
        }
        ...
        if (errno != EWOULDBLOCK) {
            ...
            throw std::system_error(errno, std::system_category(), "Failed to acquire lock");
        }
