...
void signal_handler(int signal) {
    if (signal == SIGINT) {
        picoradar::server::CLILogAdapter::addLogEntry("INFO", "收到SIGINT信号，正在关闭...");
        g_stop_signal = true;
    }
}
...
auto main(int argc, char* argv[]) -> int {
    ...
    for (int i = 1; i < argc; ++i) {
        if (std::string(argv[i]) == "--traditional" || std::string(argv[i]) == "-t") {
    ...
    try {
        ...
        if (use_traditional_cli) {
            ...
            picoradar::server::CLILogAdapter::addLogEntry("ERROR", 
    ...
    if (use_traditional_cli) {
        ...
        picoradar::server::CLILogAdapter::addLogEntry("INFO", "PICO Radar 服务器启动中...");
    }
    ...
    if (!config_result.has_value()) {
        std::string warning_msg = std::string("配置文件加载失败，使用默认配置: ") + 
            ...
            if (std::string(argv[i]) != "--traditional" && std::string(argv[i]) != "-t") {
                try {
                    ...
                    picoradar::server::CLILogAdapter::addLogEntry("INFO", 
                        "使用命令行指定端口: " + std::to_string(port));
                    break;
                } catch (const std::exception& e) {
                    picoradar::server::CLILogAdapter::addLogEntry("ERROR", 
                        "端口号无效，使用默认端口 " + std::to_string(port));
                }
            }
        }
    ...
    if (!use_traditional_cli) {
        ...
        g_cli_interface->setCommandHandler([&](const std::string& command) {
            if (command == "status") {
                ...
                picoradar::server::CLILogAdapter::addLogEntry("INFO", 
                    "可用命令: status, connections, restart, help");
    ...
    if (!use_traditional_cli) {
        stats_thread = std::thread([&]() {
            while (!g_stop_signal) {
                ...
                g_cli_interface->updateMessageStats(messages_received.load(), messages_sent.load());
