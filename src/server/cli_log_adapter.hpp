...
class CLILogAdapter {
public:
    static void initialize(std::shared_ptr<CLIInterface> cli_interface);
    static void shutdown();
    
    // 设置额外的日志处理器（例如文件日志）
    static void setAdditionalHandler(std::function<void(const std::string&, const std::string&)> handler);
...
private:
    ...
    static std::function<void(const std::string&, const std::string&)> additional_handler_;
};
