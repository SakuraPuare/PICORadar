...
public:
        ...
        ws_.set_option(websocket::stream_base::decorator(
            [](websocket::response_type& res) {
                res.set(http::field::server,
                    std::string(BOOST_BEAST_VERSION_STRING) + " picoradar-websocket-server");
            }));
    ...
    void on_accept(beast::error_code ec) {
        ...
        LOG(INFO) << "Client connected: " << ws_.next_layer().socket().remote_endpoint().address().to_string();
...
void WebsocketServer::run(const std::string& address_str, uint16_t port, int threads_count) {
