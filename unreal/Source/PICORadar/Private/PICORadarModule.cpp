...
void FPICORadarModule::StartupModule() {
  ...
  if (bClientLibraryInitialized) {
    UE_LOG(LogPICORadar, Log,
           TEXT("PICORadar module startup completed successfully"));
    UE_LOG(LogPICORadar, Log, TEXT("Client library version: %s"),
           *GetClientLibraryVersion());
  } else {
    UE_LOG(LogPICORadar, Error,
           TEXT("PICORadar module startup failed - client library "
...
void FPICORadarModule::InitializeClientLibrary() {
  try {
    ...
    UE_LOG(LogPICORadar, Log,
           TEXT("PICORadar client library initialized successfully"));
  } catch (const std::exception& e) {
    UE_LOG(LogPICORadar, Error,
           TEXT("Failed to initialize PICORadar client library: %s"),
           UTF8_TO_TCHAR(e.what()));
    bClientLibraryInitialized = false;
  } catch (...) {
    UE_LOG(
        LogPICORadar, Error,
        TEXT("Failed to initialize PICORadar client library: Unknown error"));
    bClientLibraryInitialized = false;
  }
}
...
void FPICORadarModule::CleanupClientLibrary() {
  if (bClientLibraryInitialized) {
    ...
    UE_LOG(LogPICORadar, Log,
           TEXT("PICORadar client library cleanup completed"));
  }
}
