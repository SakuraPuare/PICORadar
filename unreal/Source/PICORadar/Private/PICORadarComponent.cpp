...
void UPICORadarComponent::BeginPlay()
{
    ...
    LogMessage(FString::Printf(TEXT("PICORadar Component BeginPlay - PlayerId: %s"), *PlayerId));
    
    // 注册到世界子系统
    if (UWorld* World = GetWorld())
    {
        if (UPICORadarWorldSubsystem* Subsystem = World->GetSubsystem<UPICORadarWorldSubsystem>())
        {
    ...
    // 如果启用自动连接，则尝试连接
    if (bAutoConnectOnBeginPlay && !ServerAddress.IsEmpty() && !PlayerId.IsEmpty())
    {
...
void UPICORadarComponent::Tick(float DeltaTime)
{
    ...
    if (ConnectionTask.IsValid() && ConnectionTask->IsComplete())
    {
        ...
        else
        {
            ...
            LogMessage(FString::Printf(TEXT("Failed to connect to PICORadar server: %s"), *ErrorMsg), true);
...
void UPICORadarComponent::ConnectToServer(const FString& InServerAddress, const FString& InPlayerId, const FString& InAuthToken)
{
    ...
    bIsConnecting = true;
    LogMessage(FString::Printf(TEXT("Connecting to server: %s with PlayerId: %s"), *ServerAddress, *PlayerId));
    
#if WITH_PICORADAR_CLIENT
    // 设置玩家列表更新回调
    if (Client.IsValid())
    {
        Client->setOnPlayerListUpdate([this](const std::vector<picoradar::PlayerData>& Players)
        {
            // 将C++数据转换为UE数据类型
            ...
            for (const auto& Player : Players)
            {
                ...
                UEPlayer.PlayerId = FString(UTF8_TO_TCHAR(Player.player_id().c_str()));
                UEPlayer.SceneId = FString(UTF8_TO_TCHAR(Player.scene_id().c_str()));
...
void UPICORadarComponent::SendPlayerData()
{
    ...
    if (Client.IsValid())
    {
        ...
        {
            LogMessage(FString::Printf(TEXT("Sent player data: Pos(%.1f,%.1f,%.1f) Rot(%.1f,%.1f,%.1f)"), 
...
bool UPICORadarComponent::GetPlayerData(const FString& InPlayerId, FPICORadarPlayerData& OutPlayerData) const
{
    if (const FPICORadarPlayerData* FoundPlayer = CurrentPlayers.Find(InPlayerId))
    {
...
void UPICORadarComponent::HandlePlayerListUpdate(const TArray<FPICORadarPlayerData>& Players)
{
    // 检测新加入和离开的玩家
    ...
    for (const auto& Player : Players)
    {
        if (!CurrentPlayerIds.Contains(Player.PlayerId))
        {
            ...
            if (bEnableVerboseLogging)
            {
                LogMessage(FString::Printf(TEXT("Player joined: %s"), *Player.PlayerId));
            }
        }
    }
...
bool UPICORadarComponent::ShouldSendUpdate() const
{
    ...
    float RotationDelta = FMath::Abs(FRotator::ClampAxis(CurrentRotation.Yaw - LastSentRotation.Yaw)) +
                          FMath::Abs(FRotator::ClampAxis(CurrentRotation.Pitch - LastSentRotation.Pitch)) +
                          FMath::Abs(FRotator::ClampAxis(CurrentRotation.Roll - LastSentRotation.Roll));
    
    return (PositionDelta >= PositionThreshold) || (RotationDelta >= RotationThreshold);
}

void UPICORadarComponent::GetCurrentTransform(FVector& OutPosition, FRotator& OutRotation) const
{
...
FPICORadarPlayerData UPICORadarComponent::CreatePlayerDataFromTransform(const FVector& Position, const FRotator& Rotation) const
{
    return FPICORadarPlayerData::FromTransform(PlayerId, SceneId, Position, Rotation);
}

void UPICORadarComponent::LogMessage(const FString& Message, bool bIsError) const
{
    if (bIsError)
    {
...
void FPICORadarConnectionTask::StartConnection(const FString& ServerAddress, const FString& PlayerId, const FString& AuthToken)
{
#if WITH_PICORADAR_CLIENT
    if (Owner.IsValid())
    {
        ...
        if (ClientPtr)
        {
            ConnectionFuture = std::async(std::launch::async, [this, ClientPtr, ServerAddress, PlayerId, AuthToken]()
            {
                try
                {
                    auto Future = ClientPtr->connect(
...
bool FPICORadarConnectionTask::IsComplete() const
{
