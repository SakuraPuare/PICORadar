...
public:
    ...
    /** 获取世界子系统实例 */
    UFUNCTION(BlueprintPure, Category = "PICO Radar", CallInEditor = true, meta = (WorldContext = "WorldContext"))
    ...
    const TArray<UPICORadarComponent*>& GetRegisteredComponents() const { return RegisteredComponents; }
     ...
     */
    UFUNCTION(BlueprintPure, Category = "PICO Radar")
    bool FindPlayerById(const FString& PlayerId, FPICORadarPlayerData& OutPlayerData) const;
     ...
     */
    UFUNCTION(BlueprintCallable, Category = "PICO Radar")
    APICORadarPlayerVisualization* CreatePlayerVisualization(const FPICORadarPlayerData& PlayerData);
     ...
     */
    UFUNCTION(BlueprintPure, Category = "PICO Radar")
    APICORadarPlayerVisualization* GetPlayerVisualization(const FString& PlayerId) const;
...
public:
    ...
    /** 玩家离线后多久清理其可视化（秒） */
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "PICO Radar|Visualization", meta = (ClampMin = "1.0"))
    ...
    /** 服务器发现超时时间（秒） */
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "PICO Radar|Discovery", meta = (ClampMin = "1.0"))
    ...
    // 委托事件
public:
    DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnGlobalPlayerJoined, const FPICORadarPlayerData&, PlayerData);
    DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnGlobalPlayerLeft, const FString&, PlayerId);
    DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnGlobalPlayerUpdated, const FPICORadarPlayerData&, PlayerData);
    DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnServerDiscovered, const FString&, ServerAddress);
...
private:
    /** 处理组件的玩家列表更新 */
    UFUNCTION()
    void HandleComponentPlayerListUpdate(const TArray<FPICORadarPlayerData>& Players);
