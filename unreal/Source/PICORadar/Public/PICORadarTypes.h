...
struct PICORADAR_API FPICORadarVector3
{
    ...
    FPICORadarVector3(float InX, float InY, float InZ)
        : X(InX), Y(InY), Z(InZ) {}
...
struct PICORADAR_API FPICORadarPlayerData
{
    ...
    /** 从UE Transform创建 */
    static FPICORadarPlayerData FromTransform(
        ...
        int64 InTimestamp = 0)
    {
        ...
        Data.Timestamp = InTimestamp > 0 ? InTimestamp : FDateTime::Now().ToUnixTimestamp() * 1000;
        return Data;
    }
