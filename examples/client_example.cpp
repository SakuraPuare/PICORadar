...
int main() {
    // 初始化日志系统
    logger::Logger::Init("client_example", "./logs", logger::LogLevel::INFO, 10, true);
    ...
    try {
        ...
        // 异步连接到服务器
        auto future = client.connect("127.0.0.1:11451", "example_player", "pico_radar_secret_token");
        ...
        for (int i = 0; i < 5; ++i) {
            ...
            std::cout << "发送位置数据: (" << pos->x() << ", " << pos->y() << ", " << pos->z() << ")" << std::endl;
